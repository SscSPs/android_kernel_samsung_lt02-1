/*
 * PXA988 Power Management Routines
 *
...
void pxa988_pm_enter_lowpower_mode(int state)
{
...
static int pxa988_pm_enter(suspend_state_t state)
{
	...
	reg = __raw_readl(ICU_INT_CONF(IRQ_PXA988_PMIC_INT));
	if ((reg & 0x3) == 0)
		return -EAGAIN;
...
{
	pxa988_pm_enter_lowpower_mode(POWER_MODE_UDR);
	return 0;
}
...
{
	pxa988_pm_enter_lowpower_mode(POWER_MODE_CORE_INTIDLE);
}
