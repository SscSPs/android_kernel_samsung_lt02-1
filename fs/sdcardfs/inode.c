			  ...
			  umode_t mode)
{
	...
	if (need_graft_path(dentry)) {
		...
		if (err) {
			/* if the sbi->obbpath is not available, the lower_path
			 * won't be changed by setup_obb_dentry() but the lower
			 * path is saved to  its orig_path. this dentry will be
			 * revalidated later.  but now, the lower_path should
			 * be NULL
			 */
			sdcardfs_put_reset_lower_path(dentry);

			/* the newly created lower path which saved to its
			 * orig_path or the lower_path is the base obbpath.
	...
	if (make_nomedia_in_obb ||
		...
		nomedia_dir_name = d_absolute_path(&lower_path, page_buf,
						   PAGE_SIZE);
...
static int sdcardfs_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	/* no vfs_XXX operations required, cred overriding will be skipped. */
	if (!err) {
