...
void get_derived_permission(struct dentry *parent, struct dentry *dentry)
{
	 ...
	 *	  parent->d_name.name, dentry->d_name.name, parent_info->perm);
	 */
	...
	case PERM_LEGACY_PRE_ROOT:
		...
		info->perm = PERM_ROOT;
		info->userid = kstrtoul(dentry->d_name.name, 10, NULL);
		break;
	case PERM_ROOT:
	...
	case PERM_ANDROID:
		if (!strcasecmp(dentry->d_name.name, "data")) {
			 ...
			 * bind mount the appropriate user-specific
			 * path.
			 */
	...
	case PERM_ANDROID_OBB:
		...
		if (appid != 0)
			info->d_uid =
				multiuser_get_uid(parent_info->userid,
						  appid);
		info->d_mode = 00770;
		break;
