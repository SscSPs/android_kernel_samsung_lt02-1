...
#define MFS_SDA_INPUT() \
	do { \
		MFS_SDA_SET_INPUT(); \
		MFS_GPIO_DELAY_TO_HIGH(); \
	} while (0)
#define MFS_SDA_OUTPUT(a) \
	do { \
		MFS_SDA_SET_OUTPUT(a); \
		MFS_GPIO_DELAY_TO_HIGH(); \
	} while (0)
#define MFS_SDA_HIGH() \
	do { \
...
#define MFS_SDA_LOW() \
	do { \
		MFS_SDA_VAL_LOW(); \
		MFS_GPIO_DELAY_TO_LOW(); \
	} while (0)

#define MFS_SCL_INPUT() \
	do { \
		MFS_SCL_SET_INPUT(); \
		MFS_GPIO_DELAY_TO_HIGH(); \
	} while (0)
#define MFS_SCL_OUTPUT(a) \
	do { \
		MFS_SCL_SET_OUTPUT(a); \
		MFS_GPIO_DELAY_TO_HIGH(); \
	} while (0)
#define MFS_SCL_HIGH() \
	do { \
...
unsigned char mfs_gpio_i2c_read(unsigned char ucAddress, unsigned char *pucData,
				int nLength)
{
	int i;
