/*
 * drivers/input/touchscreen/mms100_isc.c - ISC(In-system programming via I2C)
 * enables MMS-100 Series sensor to be programmed while installed in a complete
 * system.
 *
...
/* TODO Figure out why it has #if 1 */
#if 1
...
static eISCRet_t mms100_get_version_info(struct i2c_client *_client)
{
	...
	for (i = 0; i < SECTION_NUM; i++) {
		...
		/*
		 *  previous core binary had 4 sections while current version
		 * contains 3 of them for compatibleness, register address was
		 * not modified so we get 1,2,3,5,6,7th data of read buffer
		 */
...
static eISCRet_t mms100_compare_version_info(struct i2c_client *_client)
{
	...
	for (i = 0; i < SECTION_NUM; i++) {
		/*(mbin_info[i].version == 0) || /*following...*/
	...
	for (i = 0; i < SECTION_NUM; i++) {
		...
		if (section_update_flag[i]) {
			...
			for (page_addr = mbin_info[i].start_addr;
				...
				/* 2012.08.30
				 * ret = mms100_i2c_write(_client, ptr_fw,
				 *			  PACKET_SIZE);
				 */
				pr_info("crc val : %X%X\n", crc[0], crc[1]);
				/* 2012.08.30 */
				ret = mms100_i2c_write(_client, g_wr_buf,
						       PACKET_SIZE);
 ...
 *	}
 */
	ret += request_firmware(&(fw_mbin[0]), "melfas/arubaTD/BOOT.fw",
				&_client->dev);
	ret += request_firmware(&(fw_mbin[1]), "melfas/arubaTD/CORE.fw",
				&_client->dev);
	ret += request_firmware(&(fw_mbin[2]), "melfas/arubaTD/CONF.fw",
				&_client->dev);
...
eISCRet_t mms100_ISC_download_mbinary(struct i2c_client *_client)
{
	...
	TSP_PhoneVersion = mbin_info[2].version;	/* 0 boot, 1 core,
							 * 2 config
							 */
	TSP_PanelVersion = ts_info[2].version;
	TSP_CorePhoneVersion = mbin_info[1].version;
	TSP_CorePanelVersion = ts_info[1].version;
	printk(KERN_DEBUG "[TSP] %s, version(%x)\n", __func__,
	       TSP_PanelVersion);
	if (TSP_PanelVersion == 0xFF) {
		printk(KERN_DEBUG"[TSP] %s, wrong version parameter!!(%x)\n",
		       __func__, TSP_PanelVersion);
		TSP_PanelVersion = 0x00;
	}
#ifdef CONFIG_MACH_ARUBA_TD
/*	printk("%d  %d  %d\n",gpio_get_value(TSP_TYPE1),
 *	gpio_get_value(TSP_TYPE2), gpio_get_value(TSP_TYPE3));
 */
	TSP_type = 1;
	if (((!gpio_get_value(TSP_TYPE1)) && (!gpio_get_value(TSP_TYPE2))
		&& gpio_get_value(TSP_TYPE3)) || ((gpio_get_value(TSP_TYPE1))
		&& gpio_get_value(TSP_TYPE2)) && gpio_get_value(TSP_TYPE3)) {
		TSP_type = 0;
		/* temp modification for 0x09 firmware update routine
		 * minkang 121012
		 */
		if (TSP_PanelVersion != 0x09) {
			mms_flash_fw2(_client, fw_name);
			printk(KERN_DEBUG "[TSP]DB updated.....\n");
			goto ISC_TEMP;
		}
		printk(KERN_DEBUG "[TSP]Don't need to DB update.....\n");
		goto ISC_TEMP;
	} else if (((!gpio_get_value(TSP_TYPE1)) && (!gpio_get_value(TSP_TYPE2))
		&& (!gpio_get_value(TSP_TYPE3)))) {
		/* temp modification for 0x07 firmware update routine
		 * minkang 121012
		 */
		if (TSP_PanelVersion != 0x07) {
			mms_flash_fw2(_client, fw_name2);
			printk(KERN_DEBUG "[TSP]G1F updated.....\n");
			goto ISC_TEMP;
		}
		printk(KERN_DEBUG "[TSP]Don't need to G1F update.....\n");
		goto ISC_TEMP;
/* temp modification for 0x10 firmware update routine minkang 121012 */
	} else if (TSP_PanelVersion < 0x14) {
			mms_flash_fw2(_client, fw_name3);
			printk(KERN_DEBUG "[TSP]G1F ITO updated.....\n");
			goto ISC_TEMP;
	} else
		goto ISC_ERROR_HANDLE;
#endif
