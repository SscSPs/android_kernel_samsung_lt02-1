...
/*#include <asm/gpio.h> /* prefer <linux/gpio.h> */
#include <linux.gpio.h>
#include <linux/pm.h>
...
/* Only One Read Only register, so word address need not be specified
 * (from Data Sheet)
 */
...
static int gp2a_i2c_write(u8 reg, u8 *value)
{
	...
	if ((gp2a_data->gp2a_prox_i2c_client == NULL) ||
	    (!gp2a_data->gp2a_prox_i2c_client->adapter)) {
		return -ENODEV;
...
static int gp2a_prox_offset(unsigned char vout)
{
	...
	if (!vout)
		...
		reg_value = detect;
	ret = gp2a_i2c_write(GP2A_REG_HYS/*0x02*/, &reg_value);
	if (ret < 0)
...
static int gp2a_prox_reset(void)
{
	...
	reg_value = 0x18;
	ret = gp2a_i2c_write(GP2A_REG_CON/*0x06*/, &reg_value);
	if (ret < 0)
			error("gp2a_i2c_write 4 failed\n");

	reg_value = 0x08;
	ret = gp2a_i2c_write(GP2A_REG_GAIN/*0x01*/, &reg_value);
	if (ret < 0)
	...
	reg_value = 0x04;
	/* was g p2a... */
	ret = p2a_i2c_write(GP2A_REG_CYCLE/*0x03*/, &reg_value);
	if (ret < 0)
...
static int gp2a_prox_mode(int enable)
{
	...
	if (1 == enable) {
		reg_value = 0x18;
		ret = gp2a_i2c_write(GP2A_REG_CON, &reg_value);
		if (ret < 0)
			error("gp2a_i2c_write 1 failed");
		...
		reg_value = 0x03;
		ret = gp2a_i2c_write(GP2A_REG_OPMOD, &reg_value);
		if (ret < 0)
			error("gp2a_i2c_write 3 failed");
...
static void gp2a_prox_work_func(struct work_struct *work)
{
	...
	/* Read VO & INT Clear */
	debug("[PROXIMITY] %s :\n", __func__);
	ret = gp2a_i2c_read((u8)(int_val), &value);
	if (ret < 0) {
	...
	input_report_abs(gp2a_data->prox_input_dev, ABS_DISTANCE,
			 ((vout == 1) ? 0 : 1));
...
static int gp2a_prox_cal_mode(char mode)
{
	...
	reg_value = 0x04;
	ret = gp2a_i2c_write(GP2A_REG_CYCLE/*0x03*/, &reg_value)
	if (ret < 0)
			error("gp2a_i2c_write 3 failed\n");
...
static int gp2a_cal_mode_read_file(char *mode)
{
	...
	err = cal_mode_filp->f_op->read(cal_mode_filp, mode, sizeof(u8),
					&cal_mode_filp->f_pos);
...
static int gp2a_cal_mode_save_file(char mode)
{
	...
	cal_mode_filp = filp_open(OFFSET_FILE_PATH, O_CREAT | O_TRUNC
				  | O_WRONLY | O_SYNC, 0666);
	if (IS_ERR(cal_mode_filp)) {
	...
	err = cal_mode_filp->f_op->write(cal_mode_filp, (char *)&mode,
				      ...
				      const char *buf, size_t size)
{
	...
	printk(KERN_INFO
	       "[GP2A] proximity_enable_store: new_value=%d mode=%d\n",
...
static ssize_t prox_open_calibration_show(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	int err = 0;
...
static long gp2a_prox_ioctl(struct file *filp, unsigned int ioctl_cmd,
			    unsigned long arg)
{
	int ret = 0;
	short data = 0;
	...
	case PROX_IOC_SET_CALIBRATION:
		...
		if (ret < 0 && ret != -ENOENT)
			printk(KERN_INFO
			       "[GP2A] gp2a_cal_mode_read_file() failed\n");
		else {
			if (cal_mode >= 0 && cal_mode <= 2) {
				gp2a_prox_cal_mode(cal_mode);
				gp2a_data->cal_mode = cal_mode;
...
static int gp2a_prox_probe(struct i2c_client *client,
			   const struct i2c_device_id *id)
{
	...
	ret = misc_register(&gp2a_prox_misc_device);
	if (ret < 0) {
	...
	input_set_abs_params(gp2a_data->prox_input_dev, ABS_DISTANCE,
			     0, 1, 0, 0);
	...
	ret = sysfs_create_group(&gp2a_data->prox_input_dev->dev.kobj,
				 &gp2a_prox_attr_group);
	if (ret) {
	...
	ret = request_irq(gp2a_data->irq, gp2a_irq_handler, IRQF_DISABLED
			  | IRQF_NO_SUSPEND , "proximity_int", NULL)
	if (ret) {
		error("GP2A request_irq failed IRQ_NO:%d", gp2a_data->irq);
		goto DESTROY_WORK_QUEUE;
	} else
		printk(KERN_INFO "[GP2A] request_irq success IRQ_NO:%d",
		       gp2a_data->irq);

	/* Device Initialisation with recommended register values from
	 * datasheet
	 */

	reg_value = 0x18;
	ret = gp2a_i2c_write(GP2A_REG_CYCLE, &reg_value);
	if (ret < 0)
		error("gp2a_i2c_write 1 failed");
...
static int __init gp2a_prox_init(void)
{
	...
	dev_t = device_create(sensors_class, NULL, 0, NULL,
			      "proximity_sensor");
