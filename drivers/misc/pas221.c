...
static const u8 reg_defaults[16] = {
	...
	B_INT_ALG_HYSTRESIS_WINDOW | B_PROX_OFFSET(0) | B_ALS_EN
		| B_ALS_RANGE_4000,
	0x00,
	B_PROX_PRST(1),
	0x00,
...
{
	pas221_i2c_write(pas221, REGS_CONFIG0,
			 reg_defaults[REGS_CONFIG0] & (0x20 ^ 0xff));
	...
	pas221_i2c_write(pas221, REGS_PROX_INT_TL,
			 reg_defaults[REGS_PROX_INT_TL]);
	pas221_i2c_write(pas221, REGS_PROX_INT_TH,
			 reg_defaults[REGS_PROX_INT_TH]);
	pas221_i2c_write(pas221, REGS_ALS_INT_TL,
			 reg_defaults[REGS_ALS_INT_TL]);
	pas221_i2c_write(pas221, REGS_ALS_INT_TLH,
			 reg_defaults[REGS_ALS_INT_TLH]);
	pas221_i2c_write(pas221, REGS_ALS_INT_TH,
			 reg_defaults[REGS_ALS_INT_TH]);
}
...
static inline void pas221_interrupt_clear(struct pas221_data *pas221)
{
	pas221_i2c_write(pas221, REGS_INTCONFIG, reg_defaults[REGS_INTCONFIG]);
}


static void pas221_change_prox_offset(struct pas221_data *pas221,
				      u8 offset_comp)
{
	u8 offset = offset_comp&(0xf<<3);
	pas221_i2c_write(pas221, REGS_CONFIG1,
			 (reg_defaults[REGS_CONFIG1] & ((0x0f << 3) ^ 0xff))
			  |offset);
}

static void pas221_reg_ps_als_enable(struct pas221_data *pas221,
				     enum ps_power_mode ps_en,
				     enum als_power_mode als_en)
{
	if (ps_en == ps_enable)
		pas221_i2c_write(pas221, REGS_CONFIG0, /* PS ON */
				 reg_defaults[REGS_CONFIG0]);
	else
		pas221_i2c_write(pas221, REGS_CONFIG0, /*PS OFF */
				 reg_defaults[REGS_CONFIG0] & (0x20 ^ 0xff));

	if (als_en == als_enable)
		pas221_i2c_write(pas221, REGS_CONFIG1,
				 reg_defaults[REGS_CONFIG1]); /* ALS ON */
	else
		pas221_i2c_write(pas221, REGS_CONFIG1, /* ALS OFF */
				 reg_defaults[REGS_CONFIG1] & (0x02^0xff));
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (proximity_value - poffset > 0)
		proximity_value -= poffset;
	else
		proximity_value = 0;
				      ...
				      const char *buf, size_t size)
{
	...
	if (new_value && !(pas221->power_state & PROXIMITY_ENABLED)) {
		...
		pr_err(TAG" xtalk : %d, poffset %d, ps_cpst %d, hys0 : %d , hys1: %d ",
		       pas221->xtalk, pas221->poffset, ps_cpst,
		       pas221->prox_hys[0], pas221->prox_hys[1]);
				      ...
				      const char *buf, size_t size)
{
	...
	if (new_value) {
		...
		if (!(pas221->power_state & PROXIMITY_ENABLED)) {
			pas221_reg_ps_als_enable(pas221, ps_disable,
						 als_enable);
			pas221->pdata->proximity_power(0);
		}
	}
...
#ifdef DEBUG
	pr_err(TAG " report_abs ABS_DISTANCE : %s",
	       (val == 0 ? "close" : "far"));
#endif
	/* 0 is close, 1 is far */
...
static int pas221_setup_reg(struct pas221_data *pas221)
{
	...
	pas221->prox_hys[1] = PROX_DET_OFFSET + poffset;
	pas221->prox_hys[2] = tmp;
	pr_err(TAG" %s: prox_off %d -- poffset %d ps_cpst %d\t", __func__,
	       val, poffset, ps_cpst);
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (device_create_file(pas221->proximity_dev, &dev_attr_prox_avg) < 0) {
		pr_err(TAG" %s: could not create device file(%s)!\n", __func__,
...
{
	struct device *dev_t;
	printk(KERN_DENUG "%s called", __func__);
#if defined(CONFIG_MACH_HENDRIX)
	int ret;
	  /*LDO Power On=============*/
	if (!VPROX_3_0_V) {
